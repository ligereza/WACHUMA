...
Verificabilidad del estímulo (hecho observable, no una promesa del proyecto): la afirmación de que la escena tridimensional está marcada `procedural-interpretation` no describe una intención de diseño futura. El rótulo existe hoy en el propio modelo de datos (`packages/shared/src/scene.ts`, `material-fixture.ts`, `material-fixture-demo.ts` y la migración `0002_scene_3d.sql`), y la malla que ese rótulo acompaña pasa un gate geométrico automatizado y reproducible por cualquier persona que clone el repositorio (`pnpm quality:topology`, sobre `scripts/validate-pachanoi-topology.mjs`): cero aristas de borde, cero aristas no-manifold, cero conflictos de orientación, cero triángulos degenerados y verificación de hash de contenido en los 10 frames de la secuencia, sin ocultar lo que ese gate no mide —autointersecciones, continuidad de costillas, continuidad C0/C1/C2, Jacobiano, aplicación única de shader—, declarado así por el propio script. El release completo del repositorio se verificó limpio el 2026-09-08 sobre la rama que este proyecto usa para la transferencia (`docs/quality/release-verification-2026-09-08.md`). Esto no demuestra H2 —esa es, precisamente, la pregunta que el estudio de campo financiado responde—: demuestra que el estímulo experimental que el estudio pondrá frente a un público es un artefacto real y auditable hoy, no una descripción de lo que existirá si el proyecto se financia.
...
- Nombre legal y RUT: **[FALTA]** -- el responsable debe completarlos antes del envío.
