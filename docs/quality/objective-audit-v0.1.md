...
| El seed limpio duplicaba una relación cultural editorial                        | Se eliminó la inserción hardcodeada duplicada; `content/cultures` es la única fuente de esa relación y usa `publicId` idempotente                | `db:verify` reproduce seed → integración → seed en PostgreSQL sin violar `cultural_relations_public_id_idx`.                                                         |
